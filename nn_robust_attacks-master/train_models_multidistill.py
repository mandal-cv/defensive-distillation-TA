...
def train_distillation(data, file_name, params, num_epochs=50, batch_size=128, train_temp=1, num_steps=6):
    """
    ...
    for n in range(1,num_steps+1):
        # train the assistant model at temperature t
        assistant = train(data, file_name+"_step"+str(n), params, num_epochs, batch_size, train_temp,
                        init=file_name+"_init")

        # evaluate the labels at temperature t
        predicted = assistant.predict(data.train_data)
        with tf.compat.v1.Session() as sess:
            y = sess.run(tf.nn.softmax(predicted/train_temp))
            print(y)
            data.train_labels = y
...
if not os.path.isdir('models_multistep'):
    os.makedirs('models_multistep')

T = sys.argv[1]
# train(CIFAR(), "models/cifar", [64, 64, 128, 128, 256, 256], num_epochs=50)
# train(MNIST(), "models/mnist", [32, 32, 64, 64, 200, 200], num_epochs=50)

train_distillation(MNIST(), "models_multistep/mnist-distilled-"+T, [32, 32, 64, 64, 200, 200],
                   num_epochs=50, train_temp=int(T))
train_distillation(CIFAR(), "models_multistep/cifar-distilled-"+T, [64, 64, 128, 128, 256, 256],
                   num_epochs=50, train_temp=int(T))
