...
def train_distillation(data, file_name, params, num_epochs=50, batch_size=128, train_temp=1):
    ...
    assistant = train(data, file_name+"_assistant", params, num_epochs, batch_size, train_temp,
                    init=file_name+"_init")
...
if not os.path.isdir('models_ta'):
    os.makedirs('models_ta')

T = sys.argv[1]
# train(CIFAR(), "models/cifar", [64, 64, 128, 128, 256, 256], num_epochs=50)
# train(MNIST(), "models/mnist", [32, 32, 64, 64, 200, 200], num_epochs=50)

train_distillation(MNIST(), "models_ta/mnist-distilled-"+T, [32, 32, 64, 64, 200, 200],
                   num_epochs=50, train_temp=int(T))
train_distillation(CIFAR(), "models_ta/cifar-distilled-"+T, [64, 64, 128, 128, 256, 256],
                   num_epochs=50, train_temp=int(T))
